defmodule OnTheMap.Crypto do
  @moduledoc """
  OnTheMap.Crypto provides cryptographic functions for the OnTheMap application.
  Including generating a unique ID for an identity, generating challenges for requests, as per Nonsense Auth protocol.
  """
  ...
  def main_key_less_secure(%T{} = password, %{ops: ops, mem: mem, salt_size: salt_size} \\ default_params()) do
  ...
  def main_key_init(%T{} = password, %{ops: ops, mem: mem, salt_size: salt_size} \\ default_params()) do
    raw_password = T.un(password)
